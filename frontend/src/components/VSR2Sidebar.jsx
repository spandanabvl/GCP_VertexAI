...
function StateBadge({ state }) {
  ...
  if (s.includes("ACTIVE"))     return <span className="vsr2-badge-active">✓ active</span>;
  if (s.includes("PROCESSING")) return <span className="vsr2-badge-processing">⟳ processing</span>;
  if (s.includes("FAILED"))     return <span className="vsr2-badge-failed">✕ failed</span>;
  return <span className="vsr2-badge-processing">⟳ {state || "..."}</span>;
}

export default function VSR2Sidebar({
  selectedCorpus,
  onSelectCorpus,
  activeConversation,
  ...
  async function fetchCorpora() {
    ...
    try {
      const res = await axios.get(`${API}/vsr2-corpora/`);
      setCorpora(res.data);
    } catch {
  ...
  async function createCorpus() {
    ...
    setError(null);
    try {
      const res = await axios.post(`${API}/vsr2-corpora/`, {
  ...
  async function deleteCorpus(corpus) {
    ...
    if (!ok) return;
    try {
      await axios.delete(`${API}/vsr2-corpora/`, { params: { corpus_name: corpus.corpus_name } });
  ...
  async function fetchFiles(corpusName) {
    ...
    try {
      const res = await axios.get(`${API}/vsr2-documents/`, { params: { corpus_name: corpusName } });
      setFiles(res.data);
    } catch {
      setFiles([]);
    } finally {
  ...
  async function uploadFile(e) {
    ...
    try {
      const res = await axios.post(`${API}/vsr2-documents/upload`, formData, {
  ...
  async function deleteFile(f) {
    ...
    if (!ok) return;
    try {
      // VS2 delete verifies file exists in corpus before deleting
      await axios.delete(`${API}/vsr2-documents/`, {
        params: {
  ...
  async function fetchConversations(corpusName) {
    ...
    try {
      const res = await axios.get(`${API}/vsr2-chat/conversations`, { params: { corpus_name: corpusName } });
  ...
  async function deleteConversation(e, conv) {
    e.stopPropagation();
    try {
      await axios.delete(`${API}/vsr2-chat/conversations/${conv.id}`);
      ...
      {activeSection === "corpora" && (
        <div className="sidebar-section">
          ...
          <input
            ...
            placeholder="e.g. my-serverless-corpus"
            value={displayName}
          ...
          {/* VS2 differentiator note */}
          {/* <div className="vsr2-serverless-note">
            ...
            Serverless — no index or endpoint setup needed. Location: us-central1.
          </div> */}
                ...
                >
                  <div className="corpus-item-inner">
                    ...
                    <div className="corpus-info">
                      ...
                      <span className="corpus-id">serverless · us-central1</span>
                    </div>
                  </div>
        ...
        <div className="sidebar-section">
          {!selectedCorpus ? (
            ...
            <>
              ...
              <div className="vsr-stream-note" style={{ borderColor: "#102030", background: "#050f1a" }}>
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
                Serverless managed index — file state: PROCESSING → ACTIVE
              </div>
