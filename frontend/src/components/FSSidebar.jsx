...
const SYNC_POLL_INTERVAL = 8000; // ms
...
export default function FSSidebar({
  ...
  // Corpora form — order: BQ Dataset, BQ Table, FOS ID, FV ID, Display Name
  ...
  const [syncStatus,    setSyncStatus]    = useState(null); // null | "sync_started" | "RUNNING" | "SUCCEEDED" | "FAILED"
  ...
  async function fetchCorpora() {
    ...
    try {
      const res = await axios.get(`${API}/fs-corpora/`);
      setCorpora(res.data);
    } catch {
  ...
  async function createCorpus() {
    if (!bqDataset.trim() || !bqTable.trim() || !fosId.trim() || !fvId.trim() || !displayName.trim()) return;
    setCreating(true);
    setError(null);
    try {
      const res = await axios.post(`${API}/fs-corpora/`, {
        display_name:            displayName.trim(),
        bq_dataset_id:           bqDataset.trim(),
        bq_table_id:             bqTable.trim(),
        feature_online_store_id: fosId.trim(),
        feature_view_id:         fvId.trim(),
      });
      setCorpora(prev => [res.data, ...prev]);
      onSelectCorpus(res.data);
      setBqDataset(""); setBqTable(""); setFosId(""); setFvId(""); setDisplayName("");
      setActiveSection("docs");
    } catch (e) {
      setError(e?.response?.data?.detail || "Failed to create corpus. Check infrastructure IDs.");
    } finally {
      setCreating(false);
    }
  }
  ...
  async function fetchFiles(corpusName) {
    ...
    try {
      const res = await axios.get(`${API}/fs-documents/`, { params: { corpus_name: corpusName } });
      setFiles(res.data);
    } catch {
      setFiles([]);
    } finally {
  ...
  function startSyncPolling() {
    if (!selectedCorpus?.feature_view_resource) return;
    clearInterval(syncPollRef.current);
    syncPollRef.current = setInterval(async () => {
      try {
        const res = await axios.get(`${API}/fs-documents/sync-status`, {
  ...
  async function uploadFile(e) {
    ...
    try {
      const res = await axios.post(`${API}/fs-documents/upload`, formData, {
        params: {
  ...
  async function fetchConversations(corpusName) {
    ...
    try {
      const res = await axios.get(`${API}/fs-chat/conversations`, { params: { corpus_name: corpusName } });
  ...
  async function deleteConversation(e, conv) {
    e.stopPropagation();
    try {
      await axios.delete(`${API}/fs-chat/conversations/${conv.id}`);
  ...
  function syncLabel() {
    ...
    if (syncStatus === "uploading")    return { text: "uploading…",    color: "#efb85a" };
    if (syncStatus === "sync_started") return { text: "sync started",  color: "#efb85a" };
    if (syncStatus.includes("RUNNING")) return { text: "syncing…",    color: "#efb85a" };
    if (syncStatus.includes("SUCCEEDED")) return { text: "sync done ✓", color: "#5aef8a" };
    if (syncStatus.includes("FAILED"))  return { text: "sync failed",  color: "#ef5a5a" };
    return { text: syncStatus, color: "#efb85a" };
  }
    ...
    <aside className="sidebar">
      ...
      {activeSection === "corpora" && (
        <div className="sidebar-section">
          ...
          {/* Order: BQ Dataset → BQ Table → FOS ID → FV ID → Display Name */}
          <div className="fs-field-label">BQ Dataset ID</div>
          <input className="dark-input" style={{ marginBottom: 6 }} placeholder="e.g. rag_dataset" value={bqDataset} onChange={e => setBqDataset(e.target.value)} />

          <div className="fs-field-label">BQ Table ID</div>
          <input className="dark-input" style={{ marginBottom: 6 }} placeholder="e.g. search_table_pdf" value={bqTable} onChange={e => setBqTable(e.target.value)} />

          <div className="fs-field-label">Feature Online Store ID</div>
          <input className="dark-input" style={{ marginBottom: 6 }} placeholder="e.g. search_online_store" value={fosId} onChange={e => setFosId(e.target.value)} />

          <div className="fs-field-label">Feature View ID</div>
          <input className="dark-input" style={{ marginBottom: 6 }} placeholder="e.g. search_view_pdf" value={fvId} onChange={e => setFvId(e.target.value)} />
          ...
          <button
            className="btn-primary btn-create-corpus"
            onClick={createCorpus}
            disabled={creating || !bqDataset.trim() || !bqTable.trim() || !fosId.trim() || !fvId.trim() || !displayName.trim()}
          >
            {creating ? <span className="spinner" /> : "Create corpus"}
          </button>

          <div className="fs-infra-note">
            BQ dataset/table and Feature Store will be auto-created if they don't exist.
          </div>
                ...
                >
                  <div className="corpus-item-inner">
                    ...
                    <div className="corpus-info">
                      ...
                      <span className="corpus-id">{c.bq_dataset_id} · {c.feature_online_store_id}</span>
                    </div>
                  </div>
        ...
        <div className="sidebar-section">
          {!selectedCorpus ? (
            ...
            <>
              ...
              {/* Sync status strip */}
              {sl && (
                <div className="fs-sync-strip" style={{ color: sl.color }}>
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/>
                  </svg>
                  Feature Store: {sl.text}
                </div>
              )}
              ...
              >
                ...
                {uploading ? (
                  <div className="upload-progress">
                    ...
                    <span>Uploading + auto-syncing…</span>
                  </div>
                ) : (
                  <>
                    ...
                    <span className="drop-sub" style={{ fontSize: 10, marginTop: 2 }}>→ auto-syncs to Feature Store</span>
                  </>
                )}
              </div>
                  ...
                  {files.map((f, i) => (
                    <li key={f.name || i} className="file-item" style={{ justifyContent: "space-between" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                        {f.status === "pending" && <span className="importing-badge">syncing…</span>}
                        {f.status === "synced"  && <span className="imported-badge">✓</span>}
